...
const ManagerResetPassword: React.FC = () => {
  ...
  const { managerId, token } = useParams<{ managerId: string; token: string }>(); // Get managerId and token from URL
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await api.post(`/manager/password-reset/${managerId}/${token}`, { password });
          ...
          <div className="flex flex-col m-4 md:mx-20 p-4 md:px-24 py-7 md:py-12 w-full max-w-5xl">
            <div className="text-start px-8 mb-8">
              <h2 className="text-4xl font-bold pb-2">
                Redefinir <span className="text-orange-600">Senha do Gestor</span>
              </h2>

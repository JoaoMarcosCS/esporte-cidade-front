...
export default function ModalidadesInscricao() {
  ...
  useEffect(() => {
    const fetchModalidadesInscritas = async () => {
      if (!user?.id) return;
      try {
        // Buscar apenas inscrições aprovadas, igual AgendaSemanal
        const response = await api.get('/enrollment', {
          params: { approved: true, userId: user.id }
        });
        ...
        // Buscar todas as inscrições (para saber as aguardando aprovação)
